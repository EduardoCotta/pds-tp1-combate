...
void draw(Players players, GameVariables variables, Scenery scenery) {
    drawScenery(scenery);
...
void handlePressShoot(Tank *tank) {
    if(!tank->missile.isMoving) {
        tank->missile.mobileCircle.speed += 4;
...
void handleKeyDown(Players *players, ALLEGRO_EVENT event, AllegroControls controls, GameVariables variables) {
    switch (event.keyboard.keycode) {
        ...
        case ALLEGRO_KEY_A:
            ...
            break;
        case ALLEGRO_KEY_Q:
            handlePressShoot(&players->tank1);
            break;
        case ALLEGRO_KEY_UP:
